...
void CameraCalibration::start()
{
	...
	help();
	Settings s;
	const string inputSettingsFile = "calibrationConfig.xml";
...
double CameraCalibration::computeReprojectionErrors(const vector<vector<Point3f> >& objectPoints, const vector<vector<Point2f> >& imagePoints, const vector<Mat>& rvecs, const vector<Mat>& tvecs, const Mat& cameraMatrix, const Mat& distCoeffs, vector<float>& perViewErrors)
{
...
void CameraCalibration::calcBoardCornerPositions(Size boardSize, float squareSize, vector<Point3f>& corners, Settings::Pattern patternType /*= Settings::CHESSBOARD*/)
{
	corners.clear();
...
void CameraCalibration::saveCameraParams(Settings& s, Size& imageSize, Mat& cameraMatrix, Mat& distCoeffs, const vector<Mat>& rvecs, const vector<Mat>& tvecs, const vector<float>& reprojErrs, const vector<vector<Point2f> >& imagePoints, double totalAvgErr)
{
...
void CameraCalibration::getIntrinsicParameters()
{
	...
	qDebug() << "Principalpoint in pixels   => rows = " << principalPoint_pixels_r << " columns = " << principalPoint_pixels_c;
...
void CameraCalibration::getExtrinsicParameters()
{
	...
	// Camera internals
	cv::Mat camera_matrix = (cv::Mat_<double>(3, 3) << focalLength, 0, principalPoint.x, 0, focalLength, principalPoint.y, 0, 0, 1);
	...
	rotationMatrix = (cv::Mat_<double>(3, 3) << rotation_vector.at<double>(0), 0, 0, 0, rotation_vector.at<double>(1), 0, 0, 0, rotation_vector.at<double>(2));
