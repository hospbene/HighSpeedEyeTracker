...
public:
	...
	cv::Mat rotation_vector = cv::Mat_<double>(3, 3, CV_64FC1);			// Rotation from CCS to WCS in axis-angle form with respect of WCS
	cv::Mat tmp_translation_vector = cv::Mat_<double>(3, 3);		// Translation from CCS to WCS in axis-angle form with respect to WCS
	...
	Point2d principalPoint;										//A point R at the intersection of the optical axis and the image plane.This point is referred to as the principal point or image center.
	double pixelPitch_mm_c;										// Distance from theoretical Image center  to measured Image Center in mm => theoreticalCenter_r - principalPoint.x;
	double pixelPitch_mm_r;										// Distance from theoretical Image center  to measured Image Center in mm = > theoreticalCenter_c - principalPoint.y;
	double principalPoint_pixels_c;								// computed principal Point in Pixel. Measured Image Center in mm divided by size of pixels => principalPoint.x / pixelSizeInMM;
	double principalPoint_pixels_r;								// computed principal Point in Pixel. Measured Image Center in mm divided by size of pixels => principalPoint.y / pixelSizeInMM;
	cv::Vec3f origin;
