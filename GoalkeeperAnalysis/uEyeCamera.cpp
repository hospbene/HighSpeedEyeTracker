...
void uEyeCamera::activateInternalMemory()
{
	...
	// Via camera handle
	int nMemoryMode;
	INT nRet2 = is_DeviceFeature(currentCam, IS_DEVICE_FEATURE_CMD_GET_MEMORY_MODE_ENABLE, &nMemoryMode, sizeof(nMemoryMode));
	...
	else
	{
		...
		INT nRet = is_DeviceFeature(1 | IS_USE_DEVICE_ID, IS_DEVICE_FEATURE_CMD_SET_MEMORY_MODE_ENABLE, (void*)&nEnable, sizeof(nEnable));
		if (nRet == IS_SUCCESS)
		{
...
QString uEyeCamera::getReturnCode(INT code)
{
	...
	case IS_AVI_ERR_NO_CODEC_AVAIL:
		name = "Die maximale Anzahl auf diesem System möglicher Instanzen wurde erreicht.Es kann keine weitere Instanz erzeugt werden.";
	...
	case IS_INVALID_CAPTURE_MODE:
		name = "Die Funktion kann im aktuellen Betriebsmodus der Kamera(Freilaufend, Trigger oder Standby) nicht ausgefuehrt werden.";
		break;

	case IS_INVALID_DEVICE_ID:
		name = "Die Device - ID ist ungueltig.Gueltige IDs beginnen bei 1 fuer USB - Kameras und bei 1001 fuer Gigabit - Ethernet - Kameras.";
		break;
	...
	case IS_INVALID_PARAMETER:
		name = "Einer der uebergebenen Parameter ist außerhalb des gueltigen Bereichs, oder fuer diesen Sensor nicht unterstuetzt, bzw.in diesem Modus nicht zugänglich.";
		break;
	...
	case IS_IO_REQUEST_FAILED:
		name = "Eine IO Anforderung des uEye Treibers schlug fehl.Eventuell passen die Versionen der Dateien ueye_api.dll(API) und ueye_usb.sys bzw.ueye_eth.sys(Treiber) nicht zusammen.";
		break;
	...
	case IS_NO_ACTIVE_IMG_MEM:
		name = "Kein aktivierter Bildspeicher vorhanden.Der Speicher muss mit der Funktion is_SetImageMem() aktiviert werden, oder es muss mit der Funktion is_AddToSequence() eine Sequenz aufgebaut werden.";
		break;
...
void uEyeCamera::setFlash()
{
	...
	if (IS_SUCCESS == is_IO(currentCam, IS_IO_CMD_GPIOS_SET_CONFIGURATION, (void*)&gpioConfiguration, sizeof(gpioConfiguration)))
	{
...
void uEyeCamera::brightenUpCapturing()
{
	...
	pval1 = 1;
	//Aktiviert / deaktiviert die Auto - Verstärkungs - Funktion bzw.bei HDR - Sensoren die Weißwert - Anpassung.
...
void uEyeCamera::setPixelclock()
{
	...
	INT nRet = is_DeviceFeature(currentCam, IS_DEVICE_FEATURE_CMD_GET_EXTENDED_PIXELCLOCK_RANGE_ENABLE_DEFAULT, (void*)&nEnable, sizeof(nEnable));
	if (nRet == IS_SUCCESS)
	{
		/* Set default value */
		nRet = is_DeviceFeature(currentCam, IS_DEVICE_FEATURE_CMD_SET_EXTENDED_PIXELCLOCK_RANGE_ENABLE, (void*)&nEnable, sizeof(nEnable));
	}
	nRet = is_DeviceFeature(currentCam, IS_DEVICE_FEATURE_CMD_GET_EXTENDED_PIXELCLOCK_RANGE_ENABLE, (void*)&nEnable, sizeof(nEnable));
	if (nRet == IS_SUCCESS)
	{
	...
	nRet = is_PixelClock(currentCam, IS_PIXELCLOCK_CMD_GET_NUMBER, (void*)&nNumberOfSupportedPixelClocks, sizeof(nNumberOfSupportedPixelClocks));
	if ((nRet == IS_SUCCESS) && (nNumberOfSupportedPixelClocks > 0))
	{

		ZeroMemory(&nPixelClockList, sizeof(nPixelClockList));
		nRet = is_PixelClock(currentCam, IS_PIXELCLOCK_CMD_GET_LIST, (void*)nPixelClockList, nNumberOfSupportedPixelClocks * sizeof(UINT));
	}
	...
	nRet = is_PixelClock(currentCam, IS_PIXELCLOCK_CMD_GET_DEFAULT, (void*)&nPixelClockDefault, sizeof(nPixelClockDefault));
...
void uEyeCamera::InitMemory()
{
	...
	{
		qDebug() << "AllocateImageMem:" << getReturnCode(is_AllocImageMem(currentCam, sensorInfo.nMaxWidth, sensorInfo.nMaxHeight, m_nBitsPerPixel, &sequenceMemoryPointer[i], &sequenceMemoryID[i]));
...
void uEyeCamera::showError()
{
	...
	INT nRet = is_CaptureStatus(currentCam, IS_CAPTURE_STATUS_INFO_CMD_GET, (void*)&CaptureStatusInfo, sizeof(CaptureStatusInfo));

	if (nRet == IS_SUCCESS)
	{
		qDebug() <<"CONVERSION FAILED: " << QString::number(CaptureStatusInfo.adwCapStatusCnt_Detail[IS_CAP_STATUS_API_CONVERSION_FAILED]);
		qDebug() <<"NO DEST MEM " << QString::number(CaptureStatusInfo.adwCapStatusCnt_Detail[IS_CAP_STATUS_API_NO_DEST_MEM]);
		qDebug() <<"IMAGE LOCKED: " << QString::number(CaptureStatusInfo.adwCapStatusCnt_Detail[IS_CAP_STATUS_API_IMAGE_LOCKED]);
		qDebug() <<"DEVICE NTO READY: " << QString::number(CaptureStatusInfo.adwCapStatusCnt_Detail[IS_CAP_STATUS_DRV_DEVICE_NOT_READY]);
		qDebug() <<"OUT OF BUFFERS: " << QString::number(CaptureStatusInfo.adwCapStatusCnt_Detail[IS_CAP_STATUS_DRV_OUT_OF_BUFFERS]);
		qDebug() <<"TRANSFER FAILED: " << QString::number(CaptureStatusInfo.adwCapStatusCnt_Detail[IS_CAP_STATUS_USB_TRANSFER_FAILED]);
		qDebug() <<"DEvice Time out: " << QString::number(CaptureStatusInfo.adwCapStatusCnt_Detail[IS_CAP_STATUS_DEV_TIMEOUT]);
		qDebug() <<"MISSED IMAGE: " << QString::number(CaptureStatusInfo.adwCapStatusCnt_Detail[IS_CAP_STATUS_DEV_MISSED_IMAGES]);
		qDebug() <<"BUFFER OVERRUN: " << QString::number(CaptureStatusInfo.adwCapStatusCnt_Detail[IS_CAP_STATUS_ETH_BUFFER_OVERRUN]);
		qDebug() <<"CAPUTER FAILURE: " << QString::number(CaptureStatusInfo.adwCapStatusCnt_Detail[IS_CAP_STATUS_DEV_FRAME_CAPTURE_FAILED]);
...
void uEyeCamera::printColorMode(int curColorMode)
{
	...
	switch (curColorMode)
	{
		...
		case IS_CM_SENSOR_RAW16:
			qDebug() << "ColorMode: Raw Bayer(16), unveränderte Sensorrohdaten fuer Monochrom - und Farbkameras, LUT - / Gamma - Funktion nicht aktiv.";
			break;
		case IS_CM_SENSOR_RAW12:
			qDebug() << "ColorMode: Raw Bayer(12), unveränderte Sensorrohdaten fuer Monochrom - und Farbkameras, LUT - / Gamma - Funktion nicht aktiv.";
			break;
		case IS_CM_SENSOR_RAW10:
			qDebug() << "ColorMode: Raw Bayer(10), unveränderte Sensorrohdaten fuer Monochrom - und Farbkameras, LUT - / Gamma - Funktion nicht aktiv.";
			break;
		case IS_CM_SENSOR_RAW8:
			qDebug() << "Raw Bayer(8), unveränderte Sensorrohdaten fuer Monochrom - und Farbkameras, LUT - / Gamma - Funktion nicht aktiv.";
			break;
...
void uEyeCamera::setGain(int newVal)
{
	...
	INT nRet = is_DeviceFeature(currentCam, IS_DEVICE_FEATURE_CMD_GET_SENSOR_SOURCE_GAIN_RANGE, (void*)&rangeSourceGain, sizeof(rangeSourceGain));
