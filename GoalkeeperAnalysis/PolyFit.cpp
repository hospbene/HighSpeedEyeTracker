...
bool PolyFit::calibrate(vector<CollectionTuple> &calibrationTuples, InputType inputType, QString &errorMsg)
{
    ...
    for (unsigned int i=0; i<calibrationTuples.size(); i++){
        leftPupil.insert(calibrationTuples[i].lEye.pupil.center - calibrationTuples[i].lEye.eyeCenter);
        rightPupil.insert(calibrationTuples[i].rEye.pupil.center - calibrationTuples[i].rEye.eyeCenter);
...
{
    Point2f estimate = evaluate(tuple.lEye.pupil.center - tuple.lEye.eyeCenter, tuple.rEye.pupil.center - tuple.rEye.eyeCenter, inputType);
